...
result=[prediction(1,1) prediction(1,2) depth(1);       % (head)
        prediction(2,1) prediction(2,2) depth(2);       % (neck)
        prediction(3,1) prediction(3,2) depth(3);       % (right shoulder)
        prediction(4,1) prediction(4,2) depth(4);       % (right elbow)
        prediction(5,1) prediction(5,2) depth(5);       % (right wrist)
        prediction(6,1) prediction(6,2) depth(6);       % (left shoulder)
        prediction(7,1) prediction(7,2) depth(7);       % (left elbow))
        prediction(8,1) prediction(8,2) depth(8);       % (left wrist)
        prediction(9,1) prediction(9,2) depth(9);       % (right hip)
        prediction(10,1) prediction(10,2) depth(10);    % (right knee)
        prediction(11,1) prediction(11,2) depth(11);    % (right ankle)
        prediction(12,1) prediction(12,2) depth(12);    % (left hip)
        prediction(13,1) prediction(13,2) depth(13);    % (left knee)
        prediction(14,1) prediction(14,2) depth(14);    % (left ankle)
        (prediction(9,1)+prediction(12,1))/2 (prediction(9,2)+prediction(12,2))/2 (depth(9)+depth(12))/2];   %(root hip)

Fdata(:,:,number+1)=result;

resultname=strcat('result/result',num2str(number),'.mat');
